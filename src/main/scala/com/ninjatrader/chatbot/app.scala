...
import slackservice.SlackBotService.buildCommandResponse
...
@main def runSocketModeApp(): Unit = {
  ...
  app.command(
    ...
    (req, ctx) => {
      ...
      LOGGER.info(s"Question: $userQuestion, User: $userName - $userId, Channel: $channelId, Channel name: $channelName")

      val friendlyResponseMessage = s"""You've asked: "*$userQuestion*"... ${postMessages(Random.nextInt(postMessages.length))}"""

      buildCommandResponse(friendlyResponseMessage).map(r => ctx.respond(r)).foreach(_ => ctx.ack())


      // Wrap the Ollama3 API into a Future to get the actual answer.
      val actualAnswer = s"<the actual answer goes here>"
      buildCommandResponse(actualAnswer).map(r => ctx.respond(r)).foreach(_ => ctx.ack())
      ctx.ack()
    }
  )
